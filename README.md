...
The MDBT50Q-CX / PCA10059 build target (the `OPK_TARGET_NORDIC_DONGLE` branches, `tools/patch_bsp_lfs.py`, `tools/extract_s140.py`, and the accompanying GitHub Actions workflow) is also vibe coded to hell and back — Claude Code drove the whole thing from "plug in a Raytac dongle" to green CI with an auto-published release in a single session. The same review caveats apply, plus one more: nobody else has run this specific configuration in anger yet, so treat it as a hobbyist side path rather than a supported target.

# Flashing on a Raytac MDBT50Q-CX / Nordic pca10059 dongle

This fork adds a build target for nRF52840 dongles running **Nordic's stock Open Bootloader** — Raytac MDBT50Q-CX (tested), Nordic pca10059, and other clones using the same `pca10059_usb_debug` bootloader. Boards with the **Adafruit UF2 bootloader** (Feather nRF52840, Nice!Nano, Pro Micro nRF52840, etc.) should use the standard [`docs/BUILD_AND_DEPLOY.md`](docs/BUILD_AND_DEPLOY.md) flow instead — this section doesn't apply to them.

Quick check whether this section applies: press RESET on your dongle while plugged in. If `lsusb` shows `1915:521f  Nordic Semiconductor ASA  Open DFU Bootloader`, it does.

## One-time setup

Install Nordic's `nrfutil` (Rust binary) plus its SDK-tools module. The legacy Python `pc-nrfutil` won't install on Python ≥ 3.11 — use the binary from Nordic instead:

```bash
# Linux x86_64 — see nordicsemi.com for macOS/Windows binaries
...
**Linux: `/dev/ttyACM*` permissions** — pick one, in decreasing convenience:

```bash
# Persistent + scoped: only relaxes perms when THIS specific bootloader plugs in.
sudo tee /etc/udev/rules.d/50-nordic-openpuck-dfu.rules <<'EOF'
SUBSYSTEM=="tty", ATTRS{idVendor}=="1915", ATTRS{idProduct}=="521f", TAG+="uaccess"
EOF
sudo udevadm control --reload && sudo udevadm trigger
# Then unplug/replug the dongle once.

# Or one-off per plug (ACL clears when the dongle re-enumerates):
sudo setfacl -m "u:${USER}:rw" "$(readlink -f /dev/serial/by-id/usb-Nordic_Semiconductor_Open_DFU_Bootloader_*-if00)"

# Or dialout membership (grants access to ALL USB serial devices, not just this one):
sudo usermod -aG dialout $USER && newgrp dialout
```

(Note: in zsh, use `"${USER}"` not `"$USER"` — the bare form gets misparsed as a `:r` modifier expansion when immediately followed by `:rw`.)

## First flash

Download **`openpuck-mdbt50q-cx.zip`** (~330 KB, SoftDevice + app) from the latest [release](../../releases).

1. **Enter DFU.** Press-and-hold RESET on the dongle, or press it while plugging in. The LED starts a slow ~1 Hz breathe. Confirm with `lsusb | grep 521f`.
2. **Flash:**
   ```bash
   nrfutil nrf5sdk-tools dfu usb-serial \
     -pkg openpuck-mdbt50q-cx.zip \
     -p /dev/serial/by-id/usb-Nordic_Semiconductor_Open_DFU_Bootloader_*-if00
   ```
   Takes ~30 s (SoftDevice + app). Auto-reboots into OpenPuck (Steam mode by default).
3. **Pair** your controller through the [WebUSB configurator](https://safijari.github.io/openpuck/) or the serial console — same as any OpenPuck target.

macOS port is `/dev/cu.usbmodem*`; Windows is `COM<N>` (Device Manager shows which).

## Subsequent updates

After the first successful flash, the dongle has S140 v6.1.1 installed. Later updates use **`openpuck-mdbt50q-cx-app.zip`** (~190 KB, app-only) from the same release — same procedure, ~15 s instead of ~30 s:

```bash
# Same "enter DFU" step (press RESET), then:
nrfutil nrf5sdk-tools dfu usb-serial \
  -pkg openpuck-mdbt50q-cx-app.zip \
  -p /dev/serial/by-id/usb-Nordic_Semiconductor_Open_DFU_Bootloader_*-if00
```

**Pairing persists.** LittleFS lives at flash `0xD9000-0xE0000` on this build and is not touched by either DFU package. `bonds.bin` / `cfg.bin` survive updates. To clear bonds intentionally, use the WebUSB "Factory erase" button or serial `ERASE-ALL` (same as any other OpenPuck target).

**Wrong package on a fresh dongle?** The Nordic bootloader rejects the app-only zip on a dongle without a SoftDevice installed yet (`sd_req=0xB6` mismatch) — accidentally using the wrong file fails cleanly, no brick. Just rerun with the full zip.

## What's different from the stock OpenPuck build

- **WebUSB "Firmware update" tab is disabled at the device.** The vendor commands that stream a UF2 image return `FWUP_ERR_STATE` on this build (`OPK_TARGET_NORDIC_DONGLE` branch in `OpenPuck/fw_update.cpp`); use `nrfutil` for updates instead. Everything else in the WebUSB configurator — pairing, mode switching, mappings, config — works normally.
- **No drag-and-drop `.uf2` flow.** The Nordic bootloader doesn't mount as mass storage.
- **RESET is a hardware reset.** No "double-tap RST" DFU trigger — a bare press and brief hold enters the bootloader.

## Recovery

Every flash write in this build is bounded strictly below `0xE0000` — the Nordic bootloader at `0xE0000-0xFE000` is never touched. If the app misbehaves or won't boot, press RESET → dongle re-enters DFU → reflash with `openpuck-mdbt50q-cx.zip` (the full zip, in case the app image is indeterminate). The bootloader is always reachable; you cannot brick it from userland.

## Building the packages locally

Prerequisites: `arduino-cli` with the Adafruit nRF52 core installed (`arduino-cli core install adafruit:nrf52 --additional-urls https://adafruit.github.io/arduino-board-index/package_adafruit_index.json`), `pip install adafruit-nrfutil`, and `nrfutil` with `nrf5sdk-tools` as above. Then:

```bash
./gen_version.sh
make build-mdbt50q-cx BUILD_PATH=build/cache OUTPUT_DIR=build/out
```

Outputs `build/out/openpuck-mdbt50q-cx.zip` (full) and `build/out/openpuck-mdbt50q-cx-app.zip` (app-only) — same files the CI workflow attaches to releases. The Makefile target patches the installed Adafruit BSP's `InternalFileSystem.cpp` to move LittleFS out of the Nordic bootloader region (see `tools/patch_bsp_lfs.py`, idempotent, has `--restore`), builds with `-DOPK_TARGET_NORDIC_DONGLE=1`, extracts the S140 v6.1.1 SoftDevice hex from the Adafruit BSP bundle (`tools/extract_s140.py`), generates a fresh ephemeral signing key, and packages both zips.
